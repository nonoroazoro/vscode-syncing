...
class Config {
    ...
    getConfigs({ full = false, load = false, showIndicator = false } = {}) {
        return new Promise((p_resolve) => {
            ...
            if (full) {
                list.unshift({ name: "settings-mac", type: "file" }, { name: "settings", type: "file" }, { name: "keybindings-mac", type: "file" }, { name: "keybindings", type: "file" });
            }
            else {
                list.unshift(this._env.isMac ?
                    { name: "settings-mac", type: "file" }
                    : { name: "settings", type: "file" }, this._env.isMac ?
            ...
            const results = [];
            const errors = [];
            async.eachSeries(list, (item, done) => {
                if (item.type === "folder") {
                    ...
                    temp = [
                        {
                            "name": item.name,
                            "path": path.join(this._env.codeUserPath, localFilename),
        ...
        return new Promise((p_resolve, p_reject) => {
            ...
            if (p_files) {
                ...
                this.getConfigs().then((configs) => {
                    ...
                    async.eachSeries(saveFiles, (item, done) => {
    ...
    removeConfigs(p_files) {
        const removed = [];
        return new Promise((p_resolve, p_reject) => {
            async.eachSeries(p_files, (item, done) => {
    ...
    loadSyncingSettings() {
        ...
        try {
            Object.assign(settings, JSON.parse(fs.readFileSync(this._env.syncingSettingPath, "utf8")));
        }
        catch (err) {
        }
        return settings;
