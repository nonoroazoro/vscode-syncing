...
[![Downloads](https://img.shields.io/visual-studio-marketplace/d/nonoroazoro.syncing)](https://marketplace.visualstudio.com/items?itemName=nonoroazoro.syncing)
...
**Syncing** _([源码](https://github.com/nonoroazoro/vscode-syncing))_ 是一个 VSCode 扩展，它能在**多台设备之间同步您的所有 VSCode 配置**（借助了 [GitHub Gist](https://gist.github.com)）。

[点击这里](#快速开始)快速开始配置吧，当然您也可以先看看[示例](#示例)哦。

> _我的目标是始终让它保持**简单可靠**，所以提需求别太过分啊 :)_
...
**默认情况下所有的快捷键都是关闭的**，您可在 `VSCode 键盘快捷方式`中指定快捷键：
...
1. _**`syncing.separateKeybindings`**_
   ...
   鉴于 VSCode 已提供 [Platform Specific Keybindings](https://code.visualstudio.com/updates/v1_27#_platform-specific-keybindings) 功能，您现在可以关闭这个功能。注意：在关闭该功能之前，请务必确保您已经手动合并了现有的`快捷键`配置。
...
在 `VSCode 用户设置`中增加 `http.proxy` 配置：

```json
"http.proxy": "http://127.0.0.1:1080"
```

此外，这里如果没有配置代理的话，`Syncing` 会尝试从系统环境中读取 `http_proxy` 和 `https_proxy` 作为代理设置。
...
1. 同步您的 VSCode 配置：
   ...
   1. **上传配置**
      ...
      1. _在上传完成后，您可以在 [GitHub Gist](https://gist.github.com) 中找到对应的 `Gist` 和 `Gist ID`。当然您也可以通过 `Edit` 和 `make it public` 操作将您的配置共享给其他人。_
